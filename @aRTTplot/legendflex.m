function varargout = legendflex(obj,varargin)
...
p.addParamValue('curVis',      'on',     @(x) validateattributes(x, {'char'}, {}));
...
legpos(1) = legpos(1) - legpos(3) ;
legpos(2) = legpos(2) - legpos(4) ;
...
% *********************************
%  dan segal flare to flip regions
%           part 1 of 2
% *********************************

for it = 1:nobj
    ti = nobj-(it-1) ;       %  first flare
    props = [textProps; textVals(it,:)];
    hnew.obj(it) = text(xtext(ti), ytext(ti), h.textstr{it}, props{:}, ...
...
for ii = 1:nsymbol
    ...
    if strcmp(get(h.obj(nobj+ii), 'type'), 'hggroup')
        ...
        if ~isempty(tag)
            [~, idx] = ismember(tag,h.textstr);
        end
        
        tdx = nobj - ( idx - 1 ) ;       %  second flare
        ...
        for ic = 1:length(chld)
            ...
            xnew = xnorm * symbolWidthPx + xsymbnew(tdx);
            ynew = ynorm * rowHeight     + ysymbnew(tdx);
    ...
    set(hnew.obj(nobj+ii), 'visible' , Opt.curVis )
...
%------------------------
    ...
    function resizefig(hfig, ed, ~)
        
        % Find legendflex axes
%         obj.prevFigSize = get( obj.fig , 'position' ) ;
...
%------------------------
    ...
    function [pos,unit]=getpos(h,fmt,href,opt)
        ...
        %   Author: Jrme Briot, Matlab 6.1.0.450 (R12.1)
